...
struct FileReader {
    ...
    mask:   [u64; 4],        // read_more will set, next will clear
    next:   usize,
...
impl FileReader {
    ...
    fn new(mmap: Mmap) -> FileReader {
        ...
        let u8x64 = u8x64::from_array( unsafe { *( buf.add(128) as *const[u8;64]) } );
        let mask3: Mask<i8, 64> = u8x64.simd_eq(mask_v1) | u8x64.simd_eq(mask_v2);
        let mask3 = mask3.to_bitmask();

        let u8x64 = u8x64::from_array( unsafe { *( buf.add(192) as *const[u8;64]) } );
        let mask4: Mask<i8, 64> = u8x64.simd_eq(mask_v1) | u8x64.simd_eq(mask_v2);
        let mask4 = mask4.to_bitmask();
        ...
        FileReader {
            ...
            next: mask1.trailing_zeros() as usize,  // 0..255
            line_begin: 0
        }
    }
    ...
    fn read_next_block(&mut self) {
        ...
        if likely(!self.eof) {
            ...
            if likely(self.cursor + 256 <= self.length) {
                let mask_v1 = u8x64::splat(b';');
                let mask_v2 = u8x64::splat(b'\n');

                let v1 = u8x64::from_array(unsafe { *(self.buf.add(self.cursor) as *const [u8; 64]) });
                let v2 = u8x64::from_array(unsafe { *(self.buf.add(self.cursor + 64) as *const [u8; 64]) });
                let v3 = u8x64::from_array(unsafe { *(self.buf.add(self.cursor + 128) as *const [u8; 64]) });
                let v4 = u8x64::from_array(unsafe { *(self.buf.add(self.cursor + 192) as *const [u8; 64]) });

                let v1_mask = (v1.simd_eq(mask_v1) | v1.simd_eq(mask_v2)).to_bitmask();
                let v2_mask = (v2.simd_eq(mask_v1) | v2.simd_eq(mask_v2)).to_bitmask();
                let v3_mask = (v3.simd_eq(mask_v1) | v3.simd_eq(mask_v2)).to_bitmask();
                let v4_mask = (v4.simd_eq(mask_v1) | v4.simd_eq(mask_v2)).to_bitmask();
                self.mask[0] = v1_mask;
                self.mask[1] = v2_mask;
                self.mask[2] = v3_mask;
                self.mask[3] = v4_mask;
                self.next = v1_mask.trailing_zeros() as usize;
            } else {
                self.read_last_block();      //
            }
        }
        else {
            self.next = 256;    // no more
        }
    }
    ...
    fn read_last_block(&mut self) {
        ...
        self.next = self.mask[0].trailing_zeros() as usize;
        self.eof = true;
    }
    ...
    #[inline]
    fn next_delimeter(&mut self) -> usize {
        let index = self.next;
        let result = index + self.cursor;
        assert!(index < 256);

        let pos1 = index / 64;
        let pos2 = index % 64;

        let mut mask = self.mask[pos1];
        if unlikely(pos2 == 63) {
            mask = 0;
        }
        else {
            mask &= u64::MAX << ((pos2 + 1) as u32);
        }
        // mask &=  u64::MAX.unbounded_shl((pos2+1) as u32);  // !(1 << pos2);
        if likely(mask != 0) {
            self.next = mask.trailing_zeros() as usize + pos1 * 64;
        }
        else if likely(pos1 < 3) {
            self.next = self.mask[pos1+1].trailing_zeros() as usize + (pos1+1) * 64;
        }
        else {
            self.read_next_block();  // may update self.cursor
        }
        assert!((self.next < 256) | self.eof);
        result
    }

    fn next(&mut self) -> Option<(&'static [u8], &'static [u8])> {
        if likely(self.eof == false) {
            assert!(self.next < 256);

            let first = self.next_delimeter() ;
            let second = self.next_delimeter();
            ...
            debug_assert!(value.len() >= 3 && value.len() <= 5);
...
#[inline(never)]
// based on ver12
pub fn ver14() -> Result<HashMap<String,(f32, f32, f32)>, Box<dyn std::error::Error>> {
